...
### 1. 虚拟化环境限制 (VM / Docker / Cloud)
在云服务器（ECS/EC2）或容器中，宿主机通常默认 **不透传 vPMU（虚拟性能监控单元）**。
* **现象**：无法统计“单进程”级别的硬件指令（如 `uops`, `flops`），`perf` 输出显示 `<not counted>`。
* **工具行为**：在这种环境下，本工具会自动采用 **System-Wide (`-a`)** 全局模式进行采集。
* **建议**：在云环境使用时，请确保该节点为**测试独占节点**（无其他高负载干扰），此时整机硬件计数约等于负载程序的消耗。
